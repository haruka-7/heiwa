...
pub fn create_article(state: &Arc<AppState>, article: NewArticle) -> Result<(), Option<String>> {
    //TODO call validate_unique_permalink
    match article.validate() {
        Ok(_) => match validate_unique_permalink(&state, &article.permalink) {
            Ok(_) => {
...
fn validate_unique_permalink(state: &Arc<AppState>, permalink: &str) -> Result<(), ValidationError> {
