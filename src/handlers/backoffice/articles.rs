    ...
    match is_author_logged(&session) {
        Ok(_) => {
            let articles: Vec<Article> =
                list_articles_api_call(session.get::<i32>("author_id").unwrap_or(0))
                    .await
                    .unwrap_or(vec![]);
            BackArticlesListTemplate { articles }.into_response()
        }
...
pub async fn new_action(
    ...
    match is_author_logged(&session) {
        Ok(_) => match create_article_api_call(&mut session, new_article).await {
